...
class SUBJCLUESproc:
    ...
    def features(self):
        features = Set()
        stemmed = self.SUBJCLUES['stemmed']
        token = self.SUBJCLUES['token']
        for word in stemmed:
            for pos in stemmed[word]:
                feature = stemmed[word][pos]['priorpolarity']
                features.add(feature)
                feature = stemmed[word][pos]['type']
                features.add(feature)
        for word in token:
            for pos in token[word]:
                feature = token[word][pos]['priorpolarity']    
                features.add(feature)
                feature = token[word][pos]['type']    
                features.add(feature)
    ...
    def tagWITHOUTPOS(self,word):
        ...
        if tmp_dic == None:
            ...
            if len(tmp_dic)==1:
                key = tmp_dic.keys()[0]
                return [word+"_"+tmp_dic[key]['type'],word+"_"+tmp_dic[key]['priorpolarity']]
            else:
                return []
    ...
    def inquireWITHOUTPOS(self,word):
        ...
        if tmp_dic == None:
            ...
            if len(tmp_dic)==1:
                key = tmp_dic.keys()[0]
                return [tmp_dic[key]['type'],tmp_dic[key]['priorpolarity']]
            else:
                return []
